...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $username, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		//$my_username = strstr($identity, '@', true);
        ...
        if($id){
            ...
            //add user detail
            $user_detail = array_merge($this->_filter_data($this->tables['users_details'], $additional_data), $uid);
            //add user detail
            $this->db->insert($this->tables['users_details'],  $user_detail  );
        }
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
                ...
                if(isset($user->id)){
                    ...
                    $this->db->select('full_name, phone, avatar, company, position');
                    $this->db->where('user_id', $user->id);
                    $this->db->limit(1);
                    $user_detail = $this->db->get($this->tables['users_details'])->row();

                    // var_dump($user_detail);
                    // die();

                    $this->set_session($user, $user_detail);
    ...
    public function user_detail($id = NULL)
	{
        ...
        $this->db->limit(1);
        $user_detail = $this->db->get($this->tables['users_details']);
	 ...
	 * @author jrmadsen67
	 */
	public function set_session($user, $user_detail)
	{
    ...
    public function get_user_detail(){
        $user_detail = $this->db->select('full_name, phone, avatar, company, position');
        $user_detail = $this->db->where('user_id', $this->session->userdata( 'user_id' ));
        $user_detail = $this->db->limit(1);
        $user_detail = $this->db->get('users_details')->row();

        return $user_detail;
    }

    public function get_user_details($id = false){

		$id || $id = $this->session->userdata('user_id');

		return $this->db->where($this->tables['users_details'].'.'.$this->join['users'], $id)
